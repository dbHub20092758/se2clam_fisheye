...
Frame::Frame(const Mat &im, const Se2& odo,
...
#else
    (*mpORBExtractor)(cv::Mat(img, cv::Rect(0,0,832, 350)), cv::Mat(), keyPoints, descriptors);
#endif
...
#else
    keyPointsNormPlane = f.keyPointsNormPlane;
#endif
...
void Frame::undistortKeyPoints(const Mat& K, const Mat& D){
    ...
    for(int i = 0; i < keyPoints.size(); i++){
        Eigen::Vector2d pt(keyPoints[i].pt.x, keyPoints[i].pt.y);

        Eigen::Vector3d pc;
        camera_->imageToWorld(pt, pc);

        keyPointsNormPlane.emplace_back(pc.x(), pc.y());
    }
#endif
