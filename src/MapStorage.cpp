...
#else
        file << "KeyPointsUn" << "[";
        for(int j = 0, jend = pKF->keyPointsNormPlane.size(); j < jend; j++) {
            cv::Point2f kp  = pKF->keyPointsNormPlane[j];
            file << "{";
            file << "pt" << kp;
//            file << "octave" << kp.octave;
//            file << "angle" << kp.angle;
...
#else
        pKF->keyPointsNormPlane.clear();
        FileNode nodeKPUn = nodeKF["KeyPointsUn"];
        {
            vector<cv::Point2f> vKPs;
