...
void LocalMapper::findCorrespd(const vector<int> &vMatched12, const vector<Point3f> &localMPs, const vector<bool>& vbGoodPrl){
    ...
    ///根据匹配对进行landmark，数据关联
    PtrKeyFrame pPrefKF = mpMap->getCurrentKF();
    if(!bNoMP) {

        ///已经有3D点的观测，直接加入到关键帧观测中
    ...
    ///局部地图到当前帧的映射，寻找更多的匹配点， 地图点加入新的观测，进行更新
    if(!bNoMP) {
...
#else
            Point3f x3d =
                    scv::triangulate(pMP->getMainMeasure(), mNewKF->keyPointsNormPlane[i],
                                     pMP->mMainKF->Tcw.rowRange(0,3),
                                     mNewKF->Tcw.rowRange(0,3));
#endif
    ...
    ///无地图点匹配的图像特征点匹配对，三角化在tracking过程中已经完成
    for(int i = 0, iend = pPrefKF->N; i < iend; i++){
        if(!pPrefKF->hasObservation(i) && vMatched12[i]>=0){         ///无观测且匹配上
