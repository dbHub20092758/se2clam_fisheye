...
//        id_raw++;
        ...
        if (sensorUpdated) {
            ...
            if (mbIsTracked) {
                ...
                UpdateLocalMap(1);
//                LOG(INFO) << "UpdateLocalMap is finish";

                DrawImgCurr();
                mImgMatch = Mat::zeros(mImgMatch.rows, mImgMatch.cols, mImgMatch.type());
//                LOG(INFO) << "DrawImgCurr is finish";

                DetectIfLost();
//                LOG(INFO) << "DetectIfLost is finish";

            }
            // Tracking lost, need loop close
            else {
                LOG(INFO) << "localizer is lostttttttttttttttttttttttttttt";
...
//                        LOG(INFO) << "DoLocalBA is finish";

                    }
                    else {
                        ResetLocalMap();
                        LOG(INFO) << "ResetLocalMap is finishhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh";
                    }

//                    LOG(INFO) << "DrawImgCurr is start";
                    DrawImgCurr();
//                    LOG(INFO) << "DrawImgCurr is end";

                    DrawImgMatch(mapMatchGood);
//                    LOG(INFO) << "DrawImgMatch is end";
...
void Localizer::ReadFrameInfo(const Mat &img, const Se2& odo) {
    ...
    mFrameCurr = Frame(img, odo, mpORBextractor, Config::Kcam, Config::Dcam, camera_);
...
#else
        Eigen::Vector2d uv = toVector2d( mpKFCurr->keyPointsNormPlane[ftrIdx]);
#endif
...
//        LOG(INFO) << "image curr is:" << mImgCurr.channels();
...
//        LOG(INFO) << "mpKFLoop is ok and size is:" << mImgLoop.size();
    }
    else {
        mImgCurr.copyTo(mImgLoop);
        mImgLoop.setTo(cv::Scalar(0));
//        LOG(INFO) << "mpKFLoop is cur and size is:" << mImgLoop.size();
    }

//    LOG(INFO) << "mImgLoop is empty ??:" << mImgLoop.empty();
//    LOG(INFO) << "mImgMatch is empty ??:" << mImgMatch.empty();
...
//        cvtColor(mImgMatch, mImgMatch, cv::COLOR_GRAY2BGR);
    ...
    cv::Mat mImgTemp;
    if(mImgCurr.channels() == 1)
        cv::cvtColor(mImgCurr, mImgTemp, cv::COLOR_GRAY2BGR);
    else
        mImgCurr.copyTo(mImgTemp);

    vconcat(mImgTemp, mImgLoop, mImgMatch);
...
#else
        vPtCurr.push_back(_pKFCurr->keyPointsNormPlane[idxCurr]);
        vPtLoop.push_back(_pKFLoop->keyPointsNormPlane[idxLoop]);
#endif
    }
...
void Localizer::setCamera(vslam::CameraPtr camera) {
    camera_ = camera;
}
